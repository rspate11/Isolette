...
//			setThermostatMode (" FAILURE ");
			System.out.println("Desired Minimum error, continuing with default " + min_temp);
			tStatus = true;
		}
...
//		the max_temp must be within the range [98,100] and no less than the min_temp
		if ( max_temp < Math.max(min_temp + 1, 98) || max_temp > 100 ) {
			max_temp = 100;
//			setThermostatMode (" FAILURE ");
			System.out.println("Desired Maximum error, continuing with default " + max_temp);
			tStatus = true;
		} 
...
//		the amin must be within the range [93,98] and no more than min_temp
		if ( amin < 93 || amin > Math.min(98, min_temp - 1) ) {
			amin = 93;
//			setThermostatMode (" FAILURE ");
			System.out.println("Alarm Minimum error, continuing with default " + amin);
			mStatus = true;
		} 
		
//		the amax must be within the range [99,103] and no less than max_temp
		if ( amax < Math.max(max_temp + 1, 99) || amin > 103)  {
			amax = 103;
//			setThermostatMode (" FAILURE ");
			System.out.println("Alarm Maximum error, continuing with default " + amax);
			mStatus = true;
		}
